...
void Shutdown()
{
    ...
    if (nLocalServices == NODE_BLOOM_LIGHT_ZC) {
        lightWorker.StopLightZlrgThread();
    }
#ifdef ENABLE_WALLET
    delete pwalletMain;
    pwalletMain = NULL;
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "largo.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "largod.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the LRG money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in LRG/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in LRG/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, largo, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in LRG/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Largo specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-lrgstake=<n>", strprintf(_("Enable or disable staking functionality for LRG inputs (0-1, default: %u)"), 1));
#if 0 //! \todo No Zerocoin REMOVE_LATER
    strUsage += HelpMessageOpt("-zlrgstake=<n>", strprintf(_("Enable or disable staking functionality for zLRG inputs (0-1, default: %u)"), 1));
#endif
...
#if 0 //! \todo Zerocoin - REMOVE_LATER
    ...
    strUsage += HelpMessageOpt("-backupzlrg=<n>", strprintf(_("Enable automatic wallet backups triggered after each zLRG minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zlrgbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zLRG backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
//    strUsage += "  -anonymizelargoamount=<n>     " + strprintf(_("Keep N LRG anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2019-%i The Largo Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that Largo is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Largo Core is shutting down."));
...
#endif
    // Make sure only a single Largo process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Largo Core is probably already running."), strDataDir));
...
// ********************************************************* Step 5: Backup wallet and verify wallet database integrity
    ...
    //Largo: Load Accumulator Checkpoints according to network (main/test/regtest)
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //Largo specific: zerocoin and spork DB's
                ...
                // Largo: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZLRGMinted();
                        RecalculateZLRGSpent();
                    }
                    RecalculateLRGSupply(1);
                }
                ...
                if (GetBoolArg("-reindexaccumulators", false)) {
                    if (chainActive.Height() > Params().Zerocoin_Block_V2_Start()) {
                        ...
                        // Largo: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Largo Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Largo Core to complete") << "\n";
        ...
        zwalletMain = new CzLRGWallet(pwalletMain->strWalletFile);
...
#if 0 //! \todo No Zerocoin REMOVE_LATER
        //Inititalize zLRGWallet
        uiInterface.InitMessage(_("Syncing zLRG wallet..."));

        pwalletMain->InitAutoConvertAddresses();

        bool fEnableZLrgBackups = GetBoolArg("-backupzlrg", true);
        pwalletMain->setZLrgAutoBackups(fEnableZLrgBackups);
#else
        pwalletMain->setZLrgAutoBackups(false);
#endif

        //Load zerocoin mint hashes to memory
        pwalletMain->zlrgTracker->Init();
...
#endif
    //! \todo Check how to safely remove the value below.
// XX42 Remove/refactor code below. Until then provide safe defaults
    nAnonymizeLargoAmount = 2;
...
//    }
//
//    nAnonymizeLargoAmount = GetArg("-anonymizelargoamount", 0);
//    if (nAnonymizeLargoAmount > 999999) nAnonymizeLargoAmount = 999999;
//    if (nAnonymizeLargoAmount < 2) nAnonymizeLargoAmount = 2;
    ...
    LogPrintf("Anonymize Largo Amount %d\n", nAnonymizeLargoAmount);
    ...
    /* Denominations
       ...
       For example:
       1LRG+1000 == (.1LRG+100)*10
       10LRG+10000 == (1LRG+1000)*10
    */
    ...
    */
    ...
    if (nLocalServices & NODE_BLOOM_LIGHT_ZC) {
        ...
        lightWorker.StartLightZlrgThread(threadGroup);
    }
