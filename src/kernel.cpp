...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZLrgStake(spend));
    } else {
        ...
        CLrgStake* lrgInput = new CLrgStake();
        lrgInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(lrgInput);
    }
