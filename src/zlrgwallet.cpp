...
CzLRGWallet::CzLRGWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zlrg seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZLRGSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZLRGSeed_deprecated()) {
                    LogPrintf("%s: Updated zLRG seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zlrg seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zlrg wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzLRGWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZLRGCount(nCountLastUsed);
    else if (!walletdb.ReadZLRGCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzLRGWallet::Lock()
{
    seedMaster = 0;
}

void CzLRGWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzLRGWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZLRG(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzLRGWallet::LoadMintPoolFromDB()
{
...
void CzLRGWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzLRGWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzLRGWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zlrgTracker->HasPubcoinHash(pMint.first)) {
...
bool CzLRGWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZLRG(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zlrgTracker which also adds to database
    pwalletMain->zlrgTracker->Add(dMint, true);
...
void CzLRGWallet::SeedToZLRG(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzLRGWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzLRGWallet::GenerateDeterministicZLRG(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzLRGWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzLRGWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzLRGWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
