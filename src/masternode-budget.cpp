...
{
  //! \todo Check if we have to update this for 120 sec/block interval
...
bool IsBudgetCollateralValid(uint256 nTxCollateralHash, uint256 nExpectedHash, std::string& strError, int64_t& nTime, int& nConf, bool fBudgetFinalization)
{
    ...
    BOOST_FOREACH (const CTxOut o, txCollateral.vout) {
        ...
        if (fBudgetFinalization) {
            ...
            // Note: there are still old valid budgets out there, but the check for the new 5 LRG finalization collateral
            //       will also cover the old 50 LRG finalization collateral.
...
{
#if 0 //! \todo We don't have budget payments at all. So everything goes to masternodes. Remove this later.
    int nHighestCount = -1;
