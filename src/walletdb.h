...
public:
    ...
    bool WriteZLRGSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZLRGSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZLRGSeed_deprecated(uint256& seed);
    bool EraseZLRGSeed();
    bool EraseZLRGSeed_deprecated();

    bool WriteZLRGCount(const uint32_t& nCount);
    bool ReadZLRGCount(uint32_t& nCount);
