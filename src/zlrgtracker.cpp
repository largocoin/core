...
CzLRGTracker::CzLRGTracker(std::string strWalletFile)
{
...
CzLRGTracker::~CzLRGTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzLRGTracker::Init()
{
...
bool CzLRGTracker::Archive(CMintMeta& meta)
{
...
bool CzLRGTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzLRGTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzLRGTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzLRGTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzLRGTracker::GetSerialHashes()
{
...
CAmount CzLRGTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzLRGTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzLRGTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzLRGTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzLRGTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzLRGTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzLRGTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzLRGTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzLRGTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzLRGTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzLRGTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzLRGWallet* zLRGWallet)
{
    bool iszLRGWalletInitialized = (NULL != zLRGWallet);
    CMintMeta meta;
    ...
    if (! iszLRGWalletInitialized)
        zLRGWallet = new CzLRGWallet(strWalletFile);
    meta.isSeedCorrect = zLRGWallet->CheckSeed(dMint);
    if (! iszLRGWalletInitialized)
        delete zLRGWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzLRGTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzLRGTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzLRGTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzLRGTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzLRGTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzLRGTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzLRGWallet* zLRGWallet = new CzLRGWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            Add(dMint, false, false, zLRGWallet);
        }
        delete zLRGWallet;
        LogPrint("zero", "%s: added %d dzlrg from DB\n", __func__, listDeterministicDB.size());
    }
