...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 LRG at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zLRG send
enum ZerocoinSpendStatus {
    ZLRG_SPEND_OKAY = 0,                            // No error
    ZLRG_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZLRG_WALLET_LOCKED = 2,                         // Wallet was locked
    ZLRG_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZLRG_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZLRG_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZLRG_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZLRG_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZLRG_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZLRG_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZLRG_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZLRG_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZLRG_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZLRG_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZLRG_SPENT_USED_ZLRG = 14,                      // Coin has already been spend
    ZLRG_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZLRG_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZLrgBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZLRGOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzlrgAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzLRGTracker> zlrgTracker;
    ...
    void setZWallet(CzLRGWallet* zwallet)
    {
        zwalletMain = zwallet;
        zlrgTracker = std::unique_ptr<CzLRGTracker>(new CzLRGTracker(strWalletFile));
    }

    CzLRGWallet* getZWallet() { return zwalletMain; }
    ...
    void setZLrgAutoBackups(bool fEnabled)
    {
    ...
    /** zLRG reset */
    boost::signals2::signal<void()> NotifyzLRGReset;
