...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::LRG);
    ...
    if (!settings.contains("nAnonymizeLargoAmount"))
        settings.setValue("nAnonymizeLargoAmount", 1000);
#endif

    nAnonymizeLargoAmount = settings.value("nAnonymizeLargoAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeLargoAmount"))
        SoftSetArg("-anonymizelargoamount", settings.value("nAnonymizeLargoAmount").toString().toStdString());
#endif
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in largo.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeLargoAmount:
            return QVariant(nAnonymizeLargoAmount);
#endif
        case Listen:
...
#if 0 //! \todo No Zerocoin REMOVE_LATER
        case AnonymizeLargoAmount:
            nAnonymizeLargoAmount = value.toInt();
            settings.setValue("nAnonymizeLargoAmount", nAnonymizeLargoAmount);
            emit anonymizeLargoAmountChanged(nAnonymizeLargoAmount);
            break;
#endif
        case CoinControlFeatures:
