...
#include "ui_zlrgcontroldialog.h"
...
std::set<std::string> ZLrgControlDialog::setSelectedMints;
std::set<CMintMeta> ZLrgControlDialog::setMints;

bool CZLrgControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZLrgControlDialog::COLUMN_DENOMINATION || column == ZLrgControlDialog::COLUMN_VERSION || column == ZLrgControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZLrgControlDialog::ZLrgControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZLrgControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZLrgControlDialog::~ZLrgControlDialog()
{
    delete ui;
}

void ZLrgControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZLrgControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZLrgControlWidgetItem* itemDenom(new CZLrgControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZLrgControlWidgetItem *itemMint = new CZLrgControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "The zLRG seed used to mint this zLRG is not the same as currently hold in the wallet";
            else
...
void ZLrgControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZLrgControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZLrg_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZLrgControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZLrgControlDialog::GetSelectedMints()
{
...
void ZLrgControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
